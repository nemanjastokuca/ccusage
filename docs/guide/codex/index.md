...
- **Credit equivalent** – Focused daily, monthly, and session tables show `Credits` alongside `Cost (USD)`, using one credit per `$0.04` (25 credits per dollar). Credits are calculated from the full-precision USD cost and displayed with two decimal places. JSON output remains unchanged, and `--no-cost` hides both monetary columns.
